...
__device__ inline void copySharedMemoryToGlobal(REAL* sh, REAL *func, size_t M, size_t N, size_t O, size_t global_phi, int r, int theta, int phi, size_t tp1){
    // Cada hilo copia el valor que le corresponde dentro del volumen por cada uno de los 4 tiempos
    func[I(tp1, phi, theta, r)] = sh[CI(0, tp1, threadIdx.z, threadIdx.y, threadIdx.x)];
}
